...
test('mapMarketplaceDropdown: Customer codes resolve to the right Marketplace labels', () => {
  ...
  assert.equal(mapMarketplaceDropdown('CLICKTECH_MH_BOM5_421302_RETAIL'), 'AZ RetailEZ');
...
test('marketplaceLabelFromCustomer: Amazon family + Swiggy/Flipkart short names; else customer as-is', () => {
  ...
  assert.equal(marketplaceLabelFromCustomer('COCOBLU_MH01_421302', 'AMAZON_B2B'), 'Cocoa Blue');
  assert.equal(marketplaceLabelFromCustomer('CLICKTECH_MH_BOM5_421302_RETAIL', 'AMAZON_B2B'), 'ClickTag');
...
test('phase1Mapper: Marketplace uses short Amazon/Swiggy labels, Origin City is not the warehouse', () => {
  ...
  assert.equal(row.Marketplace, 'ClickTag');
